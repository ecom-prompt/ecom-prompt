window.addEventListener('DOMContentLoaded', (event) => {
    ...
    tab.addEventListener("click",()=>{
        ...
        resetForm.addEventListener("click",resetFullForm);
    let resultText = document.querySelector(".result-text");
...
function resetContentType(){
    const contentTypeNode = document.evaluate(`//form[@id='email-form']//div[@class='wrapper-content-tabs']`, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.childNodes;
    contentTypeNode.forEach(child => {
        if(child.getAttribute('class').includes('active-tab')){
            child.setAttribute('class', child.getAttribute('class').replace(' active-tab'));
        }
    });
}
    ...
    document.querySelectorAll(className).forEach(industry => {
        if(industry.checked){
            const selectedTextField = document.evaluate(path,
                document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
            selectedTextField.innerText = 'Select';
